...
class Map extends React.Component {
	constructor(props) {
		...
		this.state = {
			...
			callbackAfterCreation: props.callbackAfterCreation,
			nameForNewPoi: "",
		...
		this.newRide = this.generateNewRide();
	...
	generateNewRide = () => {
		return new Ride(-1, "", "", undefined, undefined, []);
	}
	...
	handleRideTitleChange = (event) => {
		this.newRide.name = event.target.value;
		this.setState({ nameNewRide: event.target.value });
	}
	...
	formValidity = () => {
		return this.newRide.name!=="" && this.newRide.pois.length>1;
	}
	
	addRide = () => {
		this.newRide.creationDate = new Date();
		this.newRide.lastModificationDate = new Date();

		RideService.addRide(this.newRide);

		if(!this.standalone){
			this.state.callbackAfterCreation(this.newRide);
		}
	...
	render() {
		...
		if(this.creationMode){
			...
			<div className="pure-control-group">
				...
				value={this.state.nameNewRide} onChange={this.handleRideTitleChange}></input>
				<button className={`pure-button button-primary ${this.formValidity() ? "" : "pure-button-disabled"}`} onClick={this.addRide}>
...
Map.propTypes = {
	ride: PropTypes.any,
	callbackAfterCreation: PropTypes.func,
