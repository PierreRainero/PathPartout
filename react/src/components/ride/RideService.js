...
import rideStorage from './rideStorageMock';
...
class RideService {
    ...
    static getUserRides(user){
        let rides = rideStorage.getState().rides;
        if(rides === undefined){
            const pois1 = [];
            ...
            rideStorage.setState({ rides: rides });
        }
    ...
    /**
     * MOCKED
     * Return the number of rides for an user
     * @param {User} user 
     */
    static numberOfRide(user){
        return rideStorage.getState().rides.length;
    }
    ...
    static getRideById(id){
        const rides = rideStorage.getState().rides;
        return rides[id-1]
    }

    /**
     * MOCKED
     * Add a ride to the system
     * @param {Ride} ride 
     */
    static addRide(ride){
        const rides = rideStorage.getState().rides;

        ride.id = rides.length+1;
        rides.push(ride);
